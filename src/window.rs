//! MPEG-1 Audio Layer I synthesis window D[].
//!
//! This is the literal 512-tap polyphase synthesis window from ISO/IEC
//! 11172-3:1993 Annex B (informative) Table 3-B.3 / Annex D.1 — the
//! same table used by Layer II and Layer III.
//!
//! Values are multiples of 1/65536 (`intwinbase[j] / 65536` in the
//! libmpg123 `tabinit.c` representation); they are numerical ISO
//! constants and reproduced verbatim under the public-domain status of
//! the informative annex.
//!
//! Quick spot checks (see `tests` below):
//!   D[  0] =  0.000000000
...
mod tests {
    ...
    fn synthesis_window_endpoints() {
        ...
        assert!((SYNTHESIS_WINDOW[1] - -0.000015259).abs() < 1e-9);
        assert!((SYNTHESIS_WINDOW[64] - 0.003250122).abs() < 1e-9);
        assert!((SYNTHESIS_WINDOW[256] - 1.144989014).abs() < 1e-7);
        assert!((SYNTHESIS_WINDOW[511] - 0.000015259).abs() < 1e-9);
    }

    #[test]
    fn synthesis_window_sum_abs() {
        let s: f64 = SYNTHESIS_WINDOW.iter().map(|v| v.abs() as f64).sum();
        assert!(
            (s - 85.064).abs() < 0.5,
